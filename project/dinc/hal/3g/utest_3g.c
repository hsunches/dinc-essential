...
#define NIF_TYPE_WCDMA_UT		0x30
#define NIF_TYPE_CDMA2K_UT		0x40

#define CHECK_AND_PRINT(result,text,arg,val,invalid) (val == invalid) ? (int)strcpy(text, "invalid"): sprintf(text, arg, val) ; strcat(result, text) ;
...
{
	int value = 0;
	if (!third_g_get_supported_freq_band(&value))
	{
		sprintf(text, "%d", value);
		if (value < 1 || value > 67108863) // 2^26 -1
		{
			return UT_FAIL;
		}
		return UT_PASS;
...
int test_get_ip_address(char *text)
{
	...
	if (!third_g_get_ip_address(ipbuff))
	{
...
int test_get_working_mode(char *text)
{
	int value = 0;
	if (!third_g_get_working_mode(&value))
	{
		switch (value)
		{
			case THIRD_G_TECH_WCDMA_FDD:
				strcpy(text, "THIRD_G_TECH_WCDMA_FDD");
			break;
			case THIRD_G_TECH_WCDMA_TDD:
				strcpy(text, "THIRD_G_TECH_WCDMA_TDD");
			break;
			case THIRD_G_TECH_TD_SCDMA:
				strcpy(text, "THIRD_G_TECH_TD_SCDMA");
			break;
			case THIRD_G_TECH_HSDPA:
				strcpy(text, "THIRD_G_TECH_HSDPA");
			break;
			case THIRD_G_TECH_HSUPA:
				strcpy(text, "THIRD_G_TECH_HSUPA");
			break;
			case THIRD_G_TECH_HSPA_PLUS:
				strcpy(text, "THIRD_G_TECH_HSPA_PLUS");
			break;
			case THIRD_G_TECH_EV_DO:
				strcpy(text, "THIRD_G_TECH_EV_DO");
			break;
			case THIRD_G_TECH_EV_DO_REV_A:
				strcpy(text, "THIRD_G_TECH_EV_DO_REV_A");
			break;
			case THIRD_G_TECH_EV_DO_REV_B:
				strcpy(text, "THIRD_G_TECH_EV_DO_REV_B");
			break;
			default:
		...
		if (value < 16)
		{
			value |= NIF_TYPE_WCDMA_UT;
		}
		else
		{
			value = value >> 4;
			value |= NIF_TYPE_CDMA2K_UT;
		}

		int tech_type[9] = { 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x41, 0x42, 0x43 };
		int i = 0;
		for (; i < 9; i++)
		{
			if (value == tech_type[i])
			{
				return UT_PASS;
			}
		}
...
{
	int value = 0;
	if (!third_g_get_network_entry_latency(&value))
	{
		sprintf(text, "%d", value);
		if (value < 0)
		{
...
int test_get_rscp(char *text)
{
	int value = 0;
	if (!third_g_get_rscp(&value))
	{
		sprintf(text, "%d", value);
...
int test_get_ul_uarfcn(char *text)
{
	int value;
	if (!third_g_get_ul_uarfcn(&value))
	{
		sprintf(text, "%d", value);
		// Check UARFCN Range of Uplink
		if (value >= 12 && value <= 9888)
		{
			return UT_PASS;
		}
		return UT_FAIL;
...
int test_get_dl_uarfcn(char *text)
{
	int value;
	if (!third_g_get_dl_uarfcn(&value))
	{
		sprintf(text, "%d", value);
		// Check UARFCN Range of Downlink
		if (value >= 412 && value <= 10838)
		{
			return UT_PASS;
		}
		return UT_FAIL;
...
{
	int value = 0;
	if (!third_g_get_channel_bw(&value))
	{
		switch (value)
		{
			case THIRD_G_CHANNEL_BW_1_4:
				strcpy(text, "THIRD_G_CHANNEL_BW_1_4");
			break;
			case THIRD_G_CHANNEL_BW_3:
				strcpy(text, "THIRD_G_CHANNEL_BW_3");
			break;
			case THIRD_G_CHANNEL_BW_5:
				strcpy(text, "THIRD_G_CHANNEL_BW_5");
			break;
			case THIRD_G_CHANNEL_BW_10:
				strcpy(text, "THIRD_G_CHANNEL_BW_10");
			break;
			case THIRD_G_CHANNEL_BW_15:
				strcpy(text, "THIRD_G_CHANNEL_BW_15");
			break;
			case THIRD_G_CHANNEL_BW_20:
				strcpy(text, "THIRD_G_CHANNEL_BW_20");
			break;
			default:
				sprintf(text, "Unsupported Value:%d", value);
			break;
...
{
	int value = 0;
	if (!third_g_get_handover_latency(&value))
	{
		sprintf(text, "%d", value);
		if (value < 0)
		{
...
{
	int value = 0;
	if (!third_g_get_handover_success_count(&value))
	{
		sprintf(text, "%d", value);
		if (value < 0)
		{
...
{
	int value = 0;
	if (!third_g_get_handover_failure_count(&value))
	{
		sprintf(text, "%d", value);
		if (value < 0)
		{
...
{
	unsigned long long value = 0;
	if (!third_g_get_tx_packet_count(&value))
	{
		sprintf(text, "%lld", value);
		if (value < 0)
		{
...
{
	unsigned long long value = 0;
	if (!third_g_get_rx_packet_count(&value))
	{
		sprintf(text, "%lld", value);
		if (value < 0)
		{
...
{
	unsigned long long value = 0;
	if (!third_g_get_tx_packet_error_count(&value))
	{
		sprintf(text, "%lld", value);
		if (value < 0)
		{
...
{
	unsigned long long value = 0;
	if (!third_g_get_rx_packet_error_count(&value))
	{
		sprintf(text, "%lld", value);
		if (value < 0)
		{
...
{
	unsigned long long value = 0;
	if (!third_g_get_tx_frame_count(&value))
	{
		sprintf(text, "%lld", value);
		if (value < 0)
		{
...
{
	unsigned long long value = 0;
	if (!third_g_get_rx_frame_count(&value))
	{
		sprintf(text, "%lld", value);
		if (value < 0)
		{
...
{
	unsigned long long value = 0;
	if (!third_g_get_tx_frame_error_count(&value))
	{
		sprintf(text, "%lld", value);
		if (value < 0)
		{
...
{
	unsigned long long value = 0;
	if (!third_g_get_rx_frame_error_count(&value))
	{
		sprintf(text, "%lld", value);
		if (value < 0)
		{
...
{
	int value = 0;
	if (!third_g_get_disconnect_reason(&value))
	{
		sprintf(text, "%d", value);
		if (value < 1 || value > 9)
		{
...
int test_get_neighboring_cell_info(char *text)
{
	...
	third_g_cell_info list[MAX_3G_NGBR_CELL_CNT];

	*text = 0x00;

	int i = 0;
	for (; i < MAX_3G_NGBR_CELL_CNT; i++)
	{
		list[i].cell_id = -1;
		list[i].mcc = -1;
	...
	if (!third_g_get_neighboring_cell_info(&value, list))
	{
		int i;
		for (i = 0; i < value; ++i)
		{
			if (list[i].cell_id)
			{
				...
				check_value_d(tempstr[5], list[i].rscp, 127);

				sprintf(temptext, "CID:%s, MCC:%s, MNC:%s, LAC:%s, RSSI:%s, RSCP:%s\n", tempstr[0], tempstr[1], tempstr[2], tempstr[3], tempstr[4], tempstr[5]);
				...
				if ((list[i].rscp < -120 || list[i].rscp > -25) && list[i].rscp != 127)
				{
					return UT_FAIL;
				}
			}
...
{
	int value = 0;
	if (!third_g_get_ul_bearer_flow_max_speed(&value))
	{
		sprintf(text, "%d", value);
		if (value < 0)
		{
...
{
	int value = 0;
	if (!third_g_get_dl_bearer_flow_max_speed(&value))
	{
		sprintf(text, "%d", value);
		if (value < 0)
		{
...
int main(void)
{
	...
	add_test(p_suite, "RSCP", 3, test_get_rscp);
	add_test(p_suite, "CQI", 3, test_get_cqi);
	add_test(p_suite, "UL_UARFCN", 3, test_get_ul_uarfcn);
	add_test(p_suite, "DL_UARFCN", 3, test_get_dl_uarfcn);
